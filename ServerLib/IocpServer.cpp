...
jh::IocpServer::IocpServer(const WCHAR* serverName) : m_pConfig(nullptr), m_hCompletionPort(nullptr), m_listenSock(INVALID_SOCKET), m_pcwszServerName(serverName), m_pSessionArray(nullptr),
...
{
	if (0 == m_pConfig->m_dwConcurrentWorkerThreadCount)
	{
		SYSTEM_INFO sys;
		GetSystemInfo(&sys);

		m_pConfig->m_dwConcurrentWorkerThreadCount = sys.dwNumberOfProcessors;
	}

	_LOG(m_pcwszServerName, LOG_LEVEL_INFO, L"[Start] Concurrent ThreadCount : [%u]", m_pConfig->m_dwConcurrentWorkerThreadCount);

	m_hCompletionPort = CreateIoCompletionPort(INVALID_HANDLE_VALUE, 0, 0, m_pConfig->m_dwConcurrentWorkerThreadCount);
	...
	DWORD setLingerRet = setsockopt(m_listenSock, SOL_SOCKET, SO_LINGER, (char*)&m_pConfig->m_lingerOption, sizeof(linger));
	...
	SOCKADDR_IN sockaddrIn{ AF_INET, htons(m_pConfig->m_usPort), NetAddress::IpToAddr(m_pConfig->m_wszIp) };
...
void jh::IocpServer::Stop()
{
	...
	for (int i = 0; i < m_pConfig->m_dwConcurrentWorkerThreadCount; i++)
	{
...
{
	// 정리되지 않은 세션들을 정리
	for (int i = 0; i < m_pConfig->m_dwMaxSessionCnt; i++)
	{
...
bool jh::IocpServer::InitSessionArray(DWORD maxSessionCount)
{
	...
	m_pConfig->m_dwMaxSessionCnt = maxSessionCount;
...
void jh::IocpServer::CheckHeartbeatTimeout()
{
	...
	ULONGLONG timeoutLimit = m_pConfig->m_ullTimeoutLimit;
...
{
	if (0 != m_pConfig->m_usPort && INVALID_SOCKET != m_listenSock)
		return m_pConfig->m_usPort;
...
ServerConfig* jh::IocpServer::GetConfig()
{
	...
	// ServerConfig의 파생 형태 (MultiServerConfig)를 캐스팅
	m_pConfig = static_cast<ServerConfig*>(CreateConfig());
