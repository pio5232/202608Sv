...
void jh::JobTimer::Reserve(ULONGLONG tickAfter, std::weak_ptr<JobQueue> owner, JobRef job)
{
	const ULONGLONG executeTick = jh_utility::GetTimeStamp() + tickAfter;
	
	JobData* jobData = jh::ObjectPool<JobData>::Alloc(owner, job);
...
void jh::JobTimer::Distribute(ULONGLONG now)
{
	...
	for (TimerItem& item : items)
	{
		...
		jh::ObjectPool<JobData>::Free(item.m_pJobData);
	}

	m_bDistributing.store(false);
}

void jh::JobTimer::Clear()
{
}
