...
namespace jh_utility
{
	...
	private:
		...
		public:
			...
			const std::wstring FindItem(const WCHAR* key) const;
	...
	private:
		...
		// 주석 체크
		bool IsComment(const WCHAR* const wchPtr) const
		{
			return L'/' == *wchPtr && L'/' == *(wchPtr + 1);
		}

		// Enter, 문자열 끝 체크
		bool IsLineEnd(const WCHAR* const wchPtr) const
		{
			return L'\0' == *wchPtr || L'\n' == *wchPtr;
		}

		// 단어 체크
		bool IsMatch(const WCHAR* const wchPtr, WCHAR word) const
		{
			return word == *wchPtr;
		}
		...
		// Data 파싱 종료 플래그
		static constexpr WCHAR				kCloseSymbol = L'$';

		FILE								* m_pFile;

		WCHAR								m_szLineBuffer[kMaxLineLen];
		// 한 줄마다 파싱을 위한 버퍼.
		WCHAR								m_szParsingBuffer[kMaxLineLen];
