...
bool jh_utility::Parser::LoadFile(const WCHAR* file_name)
{
	...
	//WCHAR* fgetwsRet;
	//while ((fgetwsRet = fgetws(m_szLineBuffer, kMaxLineLen, m_pFile)) != nullptr)
	...
	// TextMode - 한 줄씩 읽는다.
	// BinaryMode - 한번에 읽어온다.
	while (nullptr != fgetws(m_szLineBuffer, kMaxLineLen, m_pFile))
	{
		switch (m_eParseType)
		{
...
void jh_utility::Parser::ParseMuchData()
{
	...
	for (int i = 0; i < kMaxLineLen - 1; i++)
	{
		...
		if (true == IsMatch(m_szLineBuffer + i, kCloseSymbol))
		{
#ifdef PRINT_DATA
...
void jh_utility::Parser::ParseData()
{
	...
	Trim(&key, kMaxLineLen);
	Trim(&value, kMaxLineLen);
#ifdef PRINT_DATA
...
jh_utility::Parser::Category::Category()
{
}
...
const std::wstring jh_utility::Parser::Category::FindItem(const WCHAR* key) const
{
