...
#include "Memory.h"
unsigned jh::DummyUpdateSystem::LogicThreadFunc(LPVOID lparam)
{
...
void jh::DummyUpdateSystem::DummyLogic(int threadNum)
{
	...
	while (1 == InterlockedOr8(&m_bRunnigFlag, 0))
	{
		...
		if (curTime - lastSendCheckTime > 1500)
		{
...
jh::DummyUpdateSystem::DummyUpdateSystem(jh::IocpClient* owner) : m_pOwner{ owner }, m_bRunnigFlag{ 1 }, m_ullRtt{}
{

}

jh::DummyUpdateSystem::~DummyUpdateSystem()
{
...
void jh::DummyUpdateSystem::Init()
{
	m_packetFuncDic.clear();

	m_packetFuncDic[jh::ROOM_LIST_RESPONSE_PACKET] = &DummyUpdateSystem::HandleRoomListResponsePacket;
	m_packetFuncDic[jh::CHAT_NOTIFY_PACKET] = &DummyUpdateSystem::HandleChatNotifyPacket;
	m_packetFuncDic[jh::CHAT_TO_ROOM_RESPONSE_PACKET] = &DummyUpdateSystem::HandleChatResponsePacket;
	m_packetFuncDic[jh::LOG_IN_RESPONSE_PACKET] = &DummyUpdateSystem::HandleLogInResponsePacket;
	m_packetFuncDic[jh::MAKE_ROOM_RESPONSE_PACKET] = &DummyUpdateSystem::HandleMakeRoomResponsePacket;
	m_packetFuncDic[jh::ENTER_ROOM_RESPONSE_PACKET] = &DummyUpdateSystem::HandleEnterRoomResponsePacket;
	m_packetFuncDic[jh::LEAVE_ROOM_RESPONSE_PACKET] = &DummyUpdateSystem::HandleLeaveRoomResponsePacket;
	m_packetFuncDic[jh::ERROR_PACKET] = &DummyUpdateSystem::HandleErrorPacket;
	m_packetFuncDic[jh::ECHO_PACKET] = &DummyUpdateSystem::HandleEchoPacket;
...
void jh::DummyUpdateSystem::Stop()
{
...
void jh::DummyUpdateSystem::EnqueueLogicEvent(DummyThreadJobRef& job, int idx)
{
...
void jh::DummyUpdateSystem::EnqueueSessionConnEvent(SessionConnectionEventRef& connectionEvent, int idx)
{
	if (idx < 0 || idx >= LOGIC_THREAD_COUNT)
		return;
...
void jh::DummyUpdateSystem::ProcessNetJob(int threadNum)
{
	PRO_START_AUTO_FUNC;

	static thread_local alignas(64) std::queue<DummyThreadJobRef> dummyJobQ;

	std::queue<DummyThreadJobRef> emptyQ;

	m_logicData[threadNum].m_netJobQueue.Swap(dummyJobQ);

	while (dummyJobQ.size() > 0)
	{
		DummyThreadJobRef& job = dummyJobQ.front();

		ProcessPacket(job->m_llSessionId, job->m_wPktType, job->m_pPacket, threadNum);
...
void jh::DummyUpdateSystem::ProcessSessionConnectionEvent(int threadNum)
{
	PRO_START_AUTO_FUNC;
	...
	while (sessionConnEventQ.size() > 0)
	{
		...
		{
		case SessionConnectionEventType::CONNECT:
		{
			DummyRef dummy = jh::MakeShared<DummyData>();
			...
			// 등록
			PacketBufferRef hbPkt = jh::DummyPacketBuilder::BuildHeartbeatPacket(dummy->m_ullLastUpdatedHeartbeatTime);

			PacketBufferRef logInReqPkt = jh::DummyPacketBuilder::BuildLoginRequestPacket();

			threadLogicData.m_pendingData.m_lLoginPacket++;
			m_pOwner->SendPacket(dummy->m_ullSessionId, hbPkt);

			SendToDummy(dummy, logInReqPkt);
		}
		break;
		case SessionConnectionEventType::DISCONNECT:
		{
			...
			DummyRef dummy = threadLogicData.m_dummyUmap[sessionId];
...
void jh::DummyUpdateSystem::ProcessDummyLogic(int threadNum)
{
	PRO_START_AUTO_FUNC;

	std::vector<DummyRef>& dummyVec = m_logicData[threadNum].m_dummyVector;

	ULONGLONG curTimeStamp = jh_utility::GetTimeStamp();

	for (DummyRef& dummy : dummyVec)
	{
		...
		if (curTimeStamp - dummy->m_ullLastUpdatedHeartbeatTime > 5000)
		{
			...
			PacketBufferRef hbPkt = jh::DummyPacketBuilder::BuildHeartbeatPacket(curTimeStamp);
		...
		case DummyStatus::IN_LOBBY:
		{
			...
			m_logicData[threadNum].m_pendingData.m_lRoomListPacket++;
		...
		case DummyStatus::IN_ROOM:
		{
			...
			if (ran < 150)
			{
				...
				m_logicData[threadNum].m_pendingData.m_lLeaveRoomPacket++;
			...
			else
			{
				...
				m_logicData[threadNum].m_pendingData.m_lChatPacket++;
...
void jh::DummyUpdateSystem::SendToDummy(DummyRef& dummy, PacketBufferRef& packet, DummyStatus nextStauts)
{
...
void jh::DummyUpdateSystem::ProcessPacket(ULONGLONG sessionId, DWORD packetType, PacketBufferRef& packet, int threadNum)
{
...
void jh::DummyUpdateSystem::HandleRoomListResponsePacket(ULONGLONG sessionId, PacketBufferRef& packet, int threadNum)
{
	PRO_START_AUTO_FUNC;

	m_logicData[threadNum].m_pendingData.m_lRoomListPacket--;
	DummyRef dummy = m_logicData[threadNum].m_dummyUmap[sessionId];
	// LOBBY 
	...
	*packet >> roomCnt;
	...
	if (roomCnt == 0)
	{
		PacketBufferRef makeRoomPkt = jh::DummyPacketBuilder::BuildMakeRoomRequestPacket();
		m_logicData[threadNum].m_pendingData.m_lMakeRoomPacket++;
		m_logicData[threadNum].m_pendingData.m_lEnterRoomPacket++;
	...
	else
	{
		...
		{
			PacketBufferRef enterRoomPkt = jh::DummyPacketBuilder::BuildEnterRoomRequestPacket(roomInfo[randRoomNum].m_usRoomNum, roomInfo[randRoomNum].m_wszRoomName);
			m_logicData[threadNum].m_pendingData.m_lEnterRoomPacket++;
...
void jh::DummyUpdateSystem::HandleLogInResponsePacket(ULONGLONG sessionId, PacketBufferRef& packet, int threadNum)
{
	PRO_START_AUTO_FUNC;
	...
	else
	{
		...
		*packet >> userId;
		...
		m_logicData[threadNum].m_pendingData.m_lLoginPacket--;
	}

}

void jh::DummyUpdateSystem::HandleMakeRoomResponsePacket(ULONGLONG sessionId, PacketBufferRef& packet, int threadNum)
{
	PRO_START_AUTO_FUNC;
	m_logicData[threadNum].m_pendingData.m_lMakeRoomPacket--;
	...
	jh::MakeRoomResponsePacket responsePkt;
...
void jh::DummyUpdateSystem::HandleEnterRoomResponsePacket(ULONGLONG sessionId, PacketBufferRef& packet, int threadNum)
{
	PRO_START_AUTO_FUNC;
	m_logicData[threadNum].m_pendingData.m_lEnterRoomPacket--;
	...
	jh::EnterRoomResponsePacket responsePkt;
...
void jh::DummyUpdateSystem::HandleChatNotifyPacket(ULONGLONG sessionId, PacketBufferRef& packet, int threadNum)
{
	PRO_START_AUTO_FUNC;
...
void jh::DummyUpdateSystem::HandleChatResponsePacket(ULONGLONG sessionId, PacketBufferRef& packet, int threadNum)
{
	PRO_START_AUTO_FUNC;
	m_logicData[threadNum].m_pendingData.m_lChatPacket--;
...
void jh::DummyUpdateSystem::HandleLeaveRoomResponsePacket(ULONGLONG sessionId, PacketBufferRef& packet, int threadNum)
{
	PRO_START_AUTO_FUNC;
	m_logicData[threadNum].m_pendingData.m_lLeaveRoomPacket--;
	// ROOM
...
void jh::DummyUpdateSystem::HandleErrorPacket(ULONGLONG sessionId, PacketBufferRef& packet, int threadNum)
{
	PRO_START_AUTO_FUNC;
	...
	*packet >> PacketErrorCode;
	...
	{
	case jh::PacketErrorCode::REQUEST_DESTROYED_ROOM: InterlockedIncrement(&agg.m_lDestroyedRoom); m_logicData[threadNum].m_pendingData.m_lEnterRoomPacket--; break;
	case jh::PacketErrorCode::REQUEST_DIFF_ROOM_NAME: InterlockedIncrement(&agg.m_lDiffRoomName); m_logicData[threadNum].m_pendingData.m_lEnterRoomPacket--; break;
	case jh::PacketErrorCode::FULL_ROOM: InterlockedIncrement(&agg.m_lFullRoom); m_logicData[threadNum].m_pendingData.m_lEnterRoomPacket--; break;
	case jh::PacketErrorCode::ALREADY_RUNNING_ROOM: InterlockedIncrement(&agg.m_lAlreadyRunningRoom); m_logicData[threadNum].m_pendingData.m_lEnterRoomPacket--; break;
	}
...
void jh::DummyUpdateSystem::HandleEchoPacket(ULONGLONG sessionId, PacketBufferRef& packet, int threadNum)
{
	// RTT 측정은 하나의 세션이 담당한다..
	PRO_START_AUTO_FUNC;

	ULONGLONG serverResponseTimestamp;
	*packet >> serverResponseTimestamp;
	...
	InterlockedExchange64((LONGLONG*)&m_ullRtt, serverResponseTimestamp - dummy->m_ullLastSendTime);

	dummy->m_ullNextActionTime = jh_utility::GetTimeStamp() + 500;
}

void jh::DummyUpdateSystem::CheckSendTimeOut(int threadNum)
{
	PRO_START_AUTO_FUNC;
...
ULONGLONG jh::DummyUpdateSystem::GetRTT() const
{
...
jh::DummyUpdateSystem::EtcData jh::DummyUpdateSystem::UpdateEtc() const
{
	EtcData etc{};
	for (int i = 0; i < LOGIC_THREAD_COUNT; i++)
	{
		...
		etc.m_pendingDataForPrint += m_logicData[i].m_pendingData;
	}

	return etc;
}
jh::DummyUpdateSystem::LogicData::LogicData() : m_errorAggregator{}, m_hLogicThread{}, m_hJobEvent{}, m_netJobQueue{}, m_sessionConnEventQueue{}, m_dummyUmap{}, m_reSendTimeoutCnt{}, m_dummyVector{} {}
