...
Transform::Transform(const Vector3& pos) : m_position(pos), m_rotation{ Vector3::Zero() }, m_fMoveSpeed(defaultWalkSpeed), m_dirNormalized(Vector3::Zero())
{
}
Transform::Transform() : m_fMoveSpeed(defaultWalkSpeed), m_rotation{ Vector3::Zero() }, m_dirNormalized(Vector3::Zero())
{
...
void Transform::SetRandomDirection()
{
...
void Transform::Move(float delta)
{
...
void Transform::SetDirection(float rotY)
{
...
bool Transform::CanGo(float prediction_x, float prediction_z) const
{
	return prediction_x > mapXMin && prediction_x < mapXMax && prediction_z > mapZMin && prediction_z < mapZMax;
}

bool Transform::IsEdge() const
{
