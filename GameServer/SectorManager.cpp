...
jh::SectorManager::SectorManager(SendPacketFunc sendPacketFunc) : m_sendPacketFunc(sendPacketFunc) {}

jh::SectorManager::~SectorManager()
{
}

bool jh::SectorManager::AddEntity(int sectorZ, int sectorX, EntityPtr entity)
{
	...
	if (entity->GetType() == jh::Entity::EntityType::GamePlayer)
		m_usAliveGamePlayerCount[sectorZ][sectorX]++;

	return true;
}

bool jh::SectorManager::DeleteEntity(EntityPtr entity, PacketBufferRef& sendBuffer)
{
...
void jh::SectorManager::SendAllEntityInfo()
{
	...
	for (int z = startZSectorPos; z < endZSectorPos; z++)
	{
		for (int x = startXSectorPos; x < endXSectorPos; x++)
		{
			...
			PacketBufferRef sendBuffer = jh::MakeShared<PacketBuffer>(sizeof(jh::MakeOtherCharacterPacket) * aroundEntityInfos.size());
...
void jh::SectorManager::UpdateSector(EntityPtr entity)
{
	...
	{
		PacketBufferRef delOtherCharacterPkt = jh::PacketBuilder::BuildDeleteOtherCharacterPacket(entity->GetEntityId());
	...
	{
		PacketBufferRef makeOtherCharacterPkt = jh::PacketBuilder::BuildMakeOtherCharacterPacket(entity->GetEntityId(), entity->GetPosition());
		PacketBufferRef moveStartNotifyPkt = jh::PacketBuilder::BuildMoveStartNotifyPacket(entity->GetEntityId(), entity->GetPosition(), entity->GetRotation().y);
	...
	for (int i = 0; i < removeAroundSector.sectorCount; i++)
	{
		...
		{
			PacketBufferRef delOtherCharacterPkt = jh::PacketBuilder::BuildDeleteOtherCharacterPacket(removeEntity->GetEntityId());
	...
	for (int i = 0; i < addAroundSector.sectorCount; i++)
	{
		for (const auto& addEntity : m_sectorSet[addAroundSector.sectors[i].m_iZ][addAroundSector.sectors[i].m_iX])
		{
			...
			PacketBufferRef makeOtherCharacterPkt = jh::PacketBuilder::BuildMakeOtherCharacterPacket(addEntity->GetEntityId(), addEntity->GetPosition());

			m_sendPacketFunc(entityId, makeOtherCharacterPkt);

			if (addEntity->IsMoving())
			{
				PacketBufferRef moveStartNotifyPkt = jh::PacketBuilder::BuildMoveStartNotifyPacket(addEntity->GetEntityId(), addEntity->GetPosition(), addEntity->GetRotation().y);
...
void jh::SectorManager::SendPacket_Sector(const Sector& sector, PacketBufferRef& packet)
{
...
void jh::SectorManager::SendPacketAroundSector(const Sector& sector, PacketBufferRef& packet)
{
...
void jh::SectorManager::SendPacketAroundSector(int sectorX, int sectorZ, PacketBufferRef& packet)
{
...
EntityPtr jh::SectorManager::GetMinEntityInRange(EntityPtr targetEntity, float range)
{
	...
	// 이 Dummy가 많아지면 이 부분에서 문제가 생길 수 있다?.
...
bool jh::SectorManager::IsValidSector(int sectorXPos, int sectorZPos)
{
	return sectorXPos >= startXSectorPos && sectorXPos < endXSectorPos &&
		sectorZPos >= startZSectorPos && sectorZPos < endZSectorPos;
}

void jh::SectorManager::GetSectorAround(int sectorXPos, int sectorZPos, AroundSector* pAroundSector)
{
	// 좌표 기준 9칸 
	// [] [] []
	// [] [] []
...
void jh::SectorManager::GetUpdatedSectorAround(EntityPtr entity, AroundSector* pRemoveAroundSector, AroundSector* pAddAroundSector)
{
