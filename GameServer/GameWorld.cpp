...
jh::GameWorld::GameWorld(UserManager* userManager, SendPacketFunc sendPacketFunc) : m_bIsUpdateRunning{ 0 }, m_fDeltaSum{ 0 }, m_pUserManager{ userManager }, m_sendPacketFunc{ sendPacketFunc }
, m_ullExpectedWinnerId{ 0 }, m_ullExpectedWinTime{ULLONG_MAX}
{
	// entity ID를 통해서 Send를 가능하게 하는 작업을 전달한다.
	auto sectorSendFunc = [this](ULONGLONG entityId, PacketBufferRef& packetPtr) {
		UserRef userPtr = m_pUserManager->GetUserByEntityId(entityId);
	...
	m_pSectorManager = jh::MakeUnique<jh::SectorManager>(sectorSendFunc);

	srand(GetCurrentThreadId());

}

jh::GameWorld::~GameWorld()
{
...
void jh::GameWorld::StartGame()
{
	InterlockedExchange8(&m_bIsUpdateRunning, 1);
...
void jh::GameWorld::Stop()
{
	InterlockedExchange8(&m_bIsUpdateRunning, 0);
...
void jh::GameWorld::Update(float deltaTime)
{
...
bool jh::GameWorld::TryEnqueueTimerAction(TimerAction&& timerAction)
{
	if (0 == InterlockedOr8(&m_bIsUpdateRunning, 0))
	{
		return false;
...
void jh::GameWorld::ProcessTimerActions()
{
...
void jh::GameWorld::CreateAI(GameWorld* worldPtr)
{
	AIPlayerPtr aiPlayer = jh::MakeShared<AIPlayer>(worldPtr);

	AddEntity(aiPlayer);

}

GamePlayerPtr jh::GameWorld::CreateGamePlayer(UserRef userPtr)
{
	GamePlayerPtr gamePlayerPtr = jh::MakeShared<jh::GamePlayer>(userPtr, this);
...
void jh::GameWorld::SendToEntity(ULONGLONG entityId, PacketBufferRef& packetPtr)
{
	UserRef userPtr = m_pUserManager->GetUserByEntityId(entityId);
...
void jh::GameWorld::BroadCast(PacketBufferRef& packetPtr)
{
	m_pUserManager->Broadcast(packetPtr);
}


void jh::GameWorld::AddEntity(EntityPtr entityPtr)
{
...
void jh::GameWorld::RemoveEntity(ULONGLONG entityId)
{
	...
	{
		// entityDic에는 존재하지만, entityToVectorDic에는 존재하지 않는 이상한 상황
	...
	PacketBufferRef sendBuffer = jh::PacketBuilder::BuildDeleteOtherCharacterPacket(entityPtr->GetEntityId());

	if (m_pSectorManager->DeleteEntity(entityPtr, sendBuffer))
		SendToSpectatorEntities(sendBuffer);
}

void jh::GameWorld::Init(USHORT total, USHORT gamePlayerCount)
{
	...
	// 초기화 내용 모두 전송했다는 의미
	PacketBufferRef sendBuffer = jh::PacketBuilder::BuildGameInitDonePacket();

	m_pUserManager->Broadcast(sendBuffer);
}

void jh::GameWorld::SetDSCount(USHORT predMaxCnt)
{
...
void jh::GameWorld::SendPacketAroundSectorNSpectators(const Sector& sector, PacketBufferRef& packet)
{
...
void jh::GameWorld::SendPacketAroundSectorNSpectators(int sectorX, int sectorZ, PacketBufferRef& packet)
{
...
void jh::GameWorld::CleanUpSpectatorEntities()
{
	auto eraseStartIter = std::remove_if(m_spectatorEntityArr.begin(), m_spectatorEntityArr.end(), [](const std::weak_ptr<jh::Entity>& weakEntity) -> bool { return weakEntity.expired(); });
...
void jh::GameWorld::SendToSpectatorEntities(PacketBufferRef& packet)
{
	// 일정 시간 간격으로 clear해주기 때문에
	// 연결이 끊겼을 경우 참조카운트가 0인 weakPtr 존재할 수 있음.
	for (const std::weak_ptr<jh::Entity>& spectatorEntity : m_spectatorEntityArr)
	{
...
void jh::GameWorld::SetSpectator(EntityPtr entity)
{
	{
		...
		PacketBufferRef spectatorInitPkt = PacketBuilder::BuildSpectatorInitPacket();
		...
		for (const EntityPtr& aliveEntityPtr : m_aliveEntityArr)
		{
			...
			PacketBufferRef makeOtherCharacterPkt = PacketBuilder::BuildMakeOtherCharacterPacket(aliveEntityId, aliveEntityPos);

			SendToEntity(entityId, makeOtherCharacterPkt);

			if (aliveEntityPtr->IsMoving())
			{
				PacketBufferRef MoveStartNotifyPkt = PacketBuilder::BuildMoveStartNotifyPacket(aliveEntityId, aliveEntityPos, aliveEntityRot.y);
...
void jh::GameWorld::CheckVictoryZoneEntry(GamePlayerPtr gamePlayerPtr)
{
		...
		false == wasInVictoryZone && isInVictoryZone)
	{
		...
		PacketBufferRef sendBuffer = jh::PacketBuilder::BuildUpdateWinnerNotifyPacket(m_ullExpectedWinnerId, m_ullExpectedWinTime);
...
void jh::GameWorld::CheckWinner()
{
	...
	if (now >= m_ullExpectedWinTime)
	{
		...
		// 게임 종료 패킷 전송
		PacketBufferRef sendBuffer = jh::PacketBuilder::BuildGameEndNotifyPacket();
...
void jh::GameWorld::InvalidateWinner(ULONGLONG userId)
{
	// 무효
	if (userId == m_ullExpectedWinnerId)
	{
		...
		PacketBufferRef sendBuffer = jh::PacketBuilder::BuildInvalidateWinnerNotifyPacket(m_ullExpectedWinnerId);

		m_pUserManager->Broadcast(sendBuffer);
	}
}

void jh::GameWorld::ProcessAttack(GamePlayerPtr attacker)
{
	// 1. 공격 상태로 전환 후 주위 모든 대상들에게 패킷 전송
	attacker->SetAttackState();
	ULONGLONG entityId = attacker->GetEntityId();
	PacketBufferRef buffer = PacketBuilder::BuildAttackNotifyPacket(entityId);
	SendPacketAroundSectorNSpectators(attacker->GetCurrentSector(), buffer);

	// 2. 데미지 판정. 공격당할 대상을 얻어온다.
	...
	{
		PacketBufferRef buffer = PacketBuilder::BuildDieNotifyPacket(victimTarget->GetEntityId());
		...
		timerAction.action = [this, victimTargetWptr]() {
			...
			if (victimTarget->GetType() == jh::Entity::EntityType::GamePlayer)
	...
	{
		PacketBufferRef buffer = PacketBuilder::BuildAttackedNotifyPacket(victimTarget->GetEntityId(), victimTarget->GetHp());
		SendPacketAroundSectorNSpectators(victimTarget->GetCurrentSector(), buffer);
	}
}
